...
export default function GastosPage() {
  ...
  async function handleSave() {
    ...
    try {
      let error
      if (editing) {
        const res = await supabase.from('expenses').update(form).eq('id', editing.id)
        error = res.error
      } else {
        const res = await supabase.from('expenses').insert(form)
        error = res.error
      }
      if (error) {
  ...
  if (showForm) {
    return (
      <div>
        ...
        <h2 className="text-xl font-bold mb-4">{editing ? 'Editar Gasto' : 'Nuevo Gasto'}</h2>
      ...
      {Object.keys(byCategory).length > 0 && (
        <div className="grid grid-cols-2 gap-3 mb-6">
          {Object.entries(byCategory).sort((a, b) => b[1] - a[1]).map(([cat, amount]) => (
            <div key={cat} className="bg-white rounded-xl shadow-sm border p-3">
            ...
            <div key={r.id} className="bg-white rounded-xl shadow-sm border p-4">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <span className="inline-block px-2 py-0.5 bg-orange-100 text-orange-800 rounded-full text-xs font-medium mt-1">
                    {r.category}
                  </span>
                  <p className="text-lg font-bold text-orange-700 mt-1">${Number(r.amount).toLocaleString('es-AR')}</p>
